...
#     global cur_map, cur_uncertainties, points_new_counter
#     points_new_counter += 1
...
#             for v in range(0, height):
#                 for u in range(0, width):
#                     if len(boundingboxs) > 0 and any((lower1 <= u <= upper1) and (lower2 <= v <= upper2) for (lower1, upper1, lower2, upper2) in
#                            boundingboxs):
#                         pipeline_depth_map[v, u] = 0.0
#                         continue
#         else:
...
#     n_frames = len(pipeline_depth_map_names)
#
#     log_file = os.path.join(args.output_path, "statistics/log.txt")
...
#                         if depth_obs <= 0:
#                             continue
#                         if len(boundingboxs) > 0 and any((lower1 <= u <= upper1) and (lower2 <= v <= upper2) for (lower1, upper1, lower2, upper2) in boundingboxs):
#                                 pipeline_depth_map[v, u] = 0.0
#                                 continue
#                         z_uncertainty = z_uncertainty_map[v, u]
...
#                         continue
#                     # not rigid
#                     if len(boundingboxs) > 0 and any((lower1 <= u_proj <= upper1) and (lower2 <= v_proj <= upper2) for (lower1, upper1, lower2, upper2) in
#                            boundingboxs):
#                         pipeline_depth_map[v_proj, u_proj] = 0.0
#                         continue
#
...
#                     th = 0.1
#
#                     if abs(depth_prior - depth_obs) >= th:
#                         continue
#                     # if dist > th:
#                     #     continue
#
...
#                         if visited[v, u] or depth_obs <= 0:
#                             continue
#                         if len(boundingboxs) > 0 and any((lower1 <= u <= upper1) and (lower2 <= v <= upper2) for
#                                (lower1, upper1, lower2, upper2) in
#                                boundingboxs):
#                             pipeline_depth_map[v, u] = 0.0
#                             continue
...
#             util_io.write_depth_map(save_fused_depth_map_path, pipeline_depth_map)
#
#             print("{0}: There are {1} points are fused and {2} points are new".format(i, points_fused_counter,
#                                                                              points_new_counter))
#             # save error map
#             # error_png = np.where(error_map >= 0.1, 255, 0).astype(np.uint8)
#             # # error_png = cv2.normalize(error_map, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
...
def compare_depth_maps(start_ID, args, pipeline_depth_map_names, flag = "raw"):
    if flag == "raw":
        stats_file = os.path.join(args.output_path, "statistics/evaluation_raw_depth_maps.txt")
    else:
        stats_file = os.path.join(args.output_path, "statistics/evaluation_fused_depth_maps.txt")
    ...
    for i in tqdm(range(start_ID, n_frames), desc="Loading and comparing raw depth maps", unit="depth maps"):
        ...
        if flag is "raw":
            ...
            fused_depth_name = "fused_depth_" + depth_name + ".pfm"
            fused_depth_map_path = os.path.join(args.output_path, fused_depth_name)
            fused_depth_map, _ = util_io.read_pfm(fused_depth_map_path, isCentimeter=False)

            pipeline_depth_map[fused_depth_map == 0.0] = 0.0
            # load bounding box
            # boundingboxs = check_rigid(boundbox_path, i)
...
if __name__ == "__main__":
    ...
    log_file = os.path.join(args.output_path, "statistics/counter.txt")
    with open(log_file, 'w') as f:
        sys.stdout = f
        for i in tqdm(range(start_ID, n_frames), desc="Start to depth fusion & a build map",
                      unit="depth maps"):
    ...
    # Evaluate depth maps
    compare_depth_maps(start_ID, args, pipeline_depth_map_names, "fused")
    compare_depth_maps(start_ID, args, pipeline_depth_map_names)
